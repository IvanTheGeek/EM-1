...
The published output uses `<base href="./" />` (dot-relative), so it works
when deployed to any subdirectory without extra configuration.
...
- **Fastmail Files** — upload the `wwwroot/` folder contents
